...
# SMART DEFAULT FEATURE: This is a helper function to make typing faster. 
# If you just hit 'Enter', it returns 0 instead of crashing the program.
def get_int_input(prompt):
    # Ask for input and remove any extra spaces
    user_input = input(prompt).strip()
    # If the user typed nothing (just hit Enter), return the number 0
    if user_input == "":
        return 0
    # Otherwise, try to turn the text into a real number
    try:
        return int(user_input)
    # If they typed letters by mistake, return -1 to trigger our error check later
    except ValueError:
        return -1 # Triggers error check in the loop
...
def main():
   ...
   # INITIAL MATH LOOP
    ...
    for p in inventory:
        ...
        while True: 
            try:
                ...
                sold = get_int_input(f"  How many {p['name']} were sold? ")
                comps = get_int_input(f"  How many {p['name']} were waste/comps? ")
                count = get_int_input(f"  What is the final physical count for {p['name']}? ")
    ...
    # Accumulators for our high-level financial summary
    ...
    # Print the Main Performance Table header
    print("\n" + "="*95)
    print(f"{'PRODUCT':<15} | {'STOCK':>6} | {'STATUS':<15} | {'SALES $':>10} | {'% OF TOTAL SALES'}")
    print("-" * 95)

    for p in inventory:
        # Track the total dollar value of all comps combined
        total_comp_value += (p["comps"] * p["unit_price"])
        # Calculate what % of the whole restaurant's sales came from this one item
        item_pct = (p["sales_dollars"] / total_net_sales * 100) if total_net_sales > 0 else 0
        
        # Traffic Light Logic: Check stock levels against the Par Level goals
        if p["final"] == 0:
            status, color = "86-OUT", "🔴"
    ...
    for s in shrinkage_list:
        # How much of this item's specific revenue was lost to shrinkage?
    ...
    # Waste Log: Items we comped or threw away
    print("\n" + "="*75)
    print(f"{'WASTE LOG (COMPS)':<25} | {'QTY':>5} | {'LOSS $':>10} | {'% OF ITEM SALES'}")
    print("-" * 75)
    for w in waste_list:
        # How much of this item's specific revenue was lost to comps?
    ...
    # --- SECTION: FINAL MONEY SUMMARY ---

    # Potential Revenue: What we WOULD have made if nothing was lost or comped
    potential_revenue = total_net_sales + total_comp_value + total_shrink_loss
    # Calculate overall percentages for the whole restaurant
